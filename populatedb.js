...
console.log(
  'This script is based on the on by MDN and can be found here: "https://raw.githubusercontent.com/hamishwillee/express-locallibrary-tutorial/master/populatedb.js". It is customized by myself to populate a shop inventory. Specified database as argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.lz91hw2.mongodb.net/inventory?retryWrites=true&w=majority"'
);
...
async function itemCreate(index, name, desc, price, number, category) {
  const itemDetail = {
    name: name,
    description: desc,
    price: price,
...
async function createItems() {
  ...
  await Promise.all([
    itemCreate(0, 
      "Volcano Map",
      "Venture into the Depths of Caves around an active Volcano and best the invading foes.",
      "1 Chaos Orb",
      1,
      categories[0] ),
    itemCreate(1, 
      "Mesa Map",
      "Round in circles around a little hill and vanquish thy enemies.",
      "10 Alchemy Orbs",
      1,
      categories[0] ),
  itemCreate(2, 
    "Dunes Map",
      "Check out the beach. But beware: you may not be alone",
      "5 Exalted Orbs",
      1,
      categories[0] ),        
  itemCreate(3, 
  ...
  itemCreate(4, 
    "Rusted Axe",
    "May still be useful for chopping wood, but that's it.",
    "1 Portal Scroll",
    1,
    categories[1] ),        
  itemCreate(5, 
  ...
  itemCreate(6, 
    "Righteous Fire",
    "For those, whose Hands get tired quickly.",
    "1 Alteration Orb",
    1,
    categories[2] ),        
  itemCreate(7, 
    "Cyclone",
    "If you want to win, you will have to spin.",
    "1 Alteration Orb",
    1,
    categories[2] ),        
  itemCreate(8, 
    "Frost Knives",
    "Careful. They won't leave behind any evidence of your deeds.",
    "1 Alteration Orb",
    1,
    categories[2] ),        
  ])
